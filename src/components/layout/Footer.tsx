      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr_1fr_1fr] gap-10">
          <div>
            ...
            <img src="/eduint-logo.png" alt="EDUINT" className="h-12 w-auto mb-5" />
            <p className="text-sm text-white/55 mb-5 max-w-[30ch] leading-relaxed">
              Your trusted AI-powered global study partner. Helping students find their perfect program since 2020.
            </p>
            <div className="flex gap-2.5">
              {SOCIALS.map((s) => (
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label}
                  className="w-9 h-9 bg-white/[0.08] hover:bg-lime text-white hover:text-ink flex items-center justify-center transition-colors">
          ...
          {Object.entries(FOOTER_LINKS).map(([title, links]) => (
            <div key={title}>
              <h3 className="text-xs font-semibold uppercase tracking-[0.1em] text-white mb-4">{title}</h3>
              <ul className="space-y-2.5">
                {links.map((link) => (
                  <li key={link.href + link.label}>
                    <Link href={link.href} className="text-sm text-white/60 hover:text-lime transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-12 pt-7 border-t border-white/10 text-center">
          <p className="text-sm text-white/40">
            &copy; {new Date().getFullYear()} Education International Bangladesh (EDUINT). All rights reserved.
          </p>
        </div>
      </div>
    </footer>
