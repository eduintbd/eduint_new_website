...
async function main() {
  ...
  // ─── Universities + Programs (loaded from seed-data) ─────
  ...
  if (!fs.existsSync(uniFile) || !fs.existsSync(progFile)) {
    console.log("  ⚠ seed-data/*.json not found — skipping universities + programs.");
    return;
  }

  const universities: UniSeed[] = JSON.parse(fs.readFileSync(uniFile, "utf8"));
  const programs: ProgramSeed[] = JSON.parse(fs.readFileSync(progFile, "utf8"));

  // Upsert universities keyed by (country, name lowercase) — survives re-seeding.
  ...
  console.log(`  universities: +${createdU} (${uniIdMap.size} total)`);
